// 역할극 초기화 — 테스트 손님↔지정 작가 대화를 처음 상태로 되돌린다.
//
//   지우는 것: 테스트 손님 2명과 그 작가 사이의 대화·메시지·예약·문의·미답변질문·검열기록
//   건드리지 않는 것: 작가 KB·안내이미지·예약서 항목, 봇 전역 정책, 실사용자 데이터
//   (대화 행 자체는 남긴다 — 지우면 방 URL 이 매번 바뀌어 불편하다)
//
// 여러 번 돌려도 안전하다. 테스트를 다시 처음부터 하고 싶을 때마다 실행.
// 사용: node scripts/reset-roleplay.cjs [작가이름|작가id]
//   예) node scripts/reset-roleplay.cjs            → 김재즈 (기본)
//       node scripts/reset-roleplay.cjs Hyun
const fs = require("fs");
...
const DEFAULT_PHOTOGRAPHER = "99d988d6-d42f-403e-b062-215d502ebc58"; // 김재즈
...
(async () => {
  ...
  // 작가 지정 — id 그대로거나 표시이름으로 찾는다
  let PHOTOGRAPHER = DEFAULT_PHOTOGRAPHER;
  let phName = "김재즈";
  if (ARG) {
    const { rows } = await c.query(
      "select id, display_name from photographers where id::text = $1 or display_name = $1",
      [ARG]
    );
    if (rows.length === 0) throw new Error(`작가를 찾지 못했어요: ${ARG}`);
    if (rows.length > 1) throw new Error(`이름이 겹쳐요 — id 로 지정해주세요: ${ARG}`);
    PHOTOGRAPHER = rows[0].id;
    phName = rows[0].display_name;
  }
  ...
  console.log(`✅ ${phName} 초기화 완료 — 대화 ${convIds.length}개, 예약 ${bookingIds.length}건 정리`);
  for (const u of users) console.log(`   · ${u.email}`);
  console.log("\n손님 진입: /chat/start?photographerId=" + PHOTOGRAPHER);
  console.log("예약 폼:   /inquiry?photographerId=" + PHOTOGRAPHER);
  await c.end();
})().catch((e) => {
