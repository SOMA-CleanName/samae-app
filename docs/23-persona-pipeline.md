...
- ⛔ **2026-09-21: 맥미니에서 qwen 을 돌리지 않기로 했다(무겁다).** `serve.py` 의 `/persona_copy` 가 기본으로 501 을
  주므로 `local` 로 켜 두어도 작문은 claude 로 넘어간다(판단만 SigLIP). 운영은 `claude` 를 쓴다(docs/28 §4, docs/38 §3-3)
