...
개발 환경에서만 `/dev/cart-meta`를 제공한다. 운영 환경에서는 `notFound()`로 접근을 막는다.

- 현재 공개 사진을 화면과 같은 장소 판정 기준(`location_text`, 없으면 `region`)으로 조회한다.
- `장소만 있음`, `가격만 있음`, `둘 다 없음` 세 그룹을 각각 최대 4장씩 표시한다.
- 각 카드에는 사진, 실제 가격·장소 값, 관심사진 상세에서 나타날 예상 문구를 표시한다.
- 각 사진을 관심사진에 담거나 원본 상세 페이지로 이동할 수 있게 해 실제 `FloatingCart` 하단 메타 패널을 확인한다.
- 데이터가 없는 그룹은 오류 대신 해당 조건의 샘플이 없다는 빈 상태를 표시한다.
- 이 페이지는 홈 피드·추천 순서·운영 사용자 화면에 영향을 주지 않는 임시 로컬 QA 도구다.
