...
### 화면과 검증은 실제 것을 그대로 쓴다

`ApplyIntro` · `ApplyLeadForm` · `AgreeGate` 를 **그 컴포넌트 그대로** 가져다 쓰고
액션만 갈아 끼웠다. 입력 검증도 실제 zod 스키마(`parseApplyForm`)다.

복제하면 둘이 조용히 어긋나고 **어긋난 쪽을 QA 하게 된다** — 그게 QA 용 화면을 따로
만들 때 생기는 진짜 비용이다. 그래서 복제하지 않았다.
