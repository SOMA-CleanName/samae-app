...
// 홈 배너 이미지 업로드 — 운영자 전용. 리사이즈 + 썸네일 생성 후 Storage 저장.
// DB 반영은 하지 않고 이미지 정보만 반환한다(배너 행 생성은 서버 액션에서).
export async function POST(req: Request) {
  const me = await getCurrentUser();
  if (!me || me.role !== "admin") {
    return Response.json({ error: "운영자만 업로드할 수 있습니다." }, { status: 403 });
  }
  ...
  try {
    ...
    // 배너는 가로로 크게 쓰이므로 사진 피드(1600)보다 넉넉하게 2000px
