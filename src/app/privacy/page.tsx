...
/*
  ...
  🔴 미확정 (법무·운영 확인 후 채울 것)
     ...
     · 개인정보 보호책임자의 **전화번호**. 사업용 회선 개통 후 추가(현재는 이메일로 충족).
       법 제30조가 "성명 또는 부서 명칭과 **전화번호 등 연락처**" 를 요구하므로 이메일로도
       충족되나, 전화번호가 있는 편이 안전하다.

  ✅ 2026-09-04 확인 완료 (법제처)
     · 제30조 — 처리방침 필수 기재: 파기절차 · 위탁 · 권리 행사방법 · 보호책임자 성명과 연락처
     · 제31조 — 보호책임자 지정. **단 소규모는 지정 면제 가능**(대통령령). 개인사업자는
       대표자가 보호책임자가 되므로 지금 표기가 맞다.
     · 제22조 — **각각의 동의 사항을 구분하여** 알리고 받을 것 → 가입 체크박스 2개의 근거
     · 제28조의8 — 국외이전 요건 5가지 중 **"계약 체결·이행을 위해 필요한 처리위탁·보관"**
       에 해당. 다만 시행령 제31조 ①2호가 **이전 근거를 처리방침에 공개**하도록 해서 §5 문장에
       조문을 명시했다.
*/
    ...
    <main className="mx-auto max-w-2xl px-5 py-10 font-kr">
      ...
      <div className="mt-8 space-y-7 text-sm leading-relaxed text-fg/85">
        ...
        <Section title="5. 개인정보의 국외 이전">
          {/*
            ⚠️ 이전 근거 규정을 문장에 명시해야 한다. 개인정보보호법 시행령 제31조 제1항 제2호가
               처리방침에 **이전 근거**를 공개하도록 요구한다(법 제28조의8 확인 과정에서 발견).
               "계약 이행에 필요한 범위" 라고만 적고 근거 조문을 빼면 요건을 반만 채운 것이다.
          */}
          <p>
            위 수탁자는 모두 국외에 소재한 사업자로, 서비스 제공을 위해 개인정보가 국외로
            이전됩니다. 이전은 <b className="font-semibold text-fg">「개인정보 보호법」 제28조의8에
            따라 이용자와의 계약을 이행하기 위하여 필요한 처리위탁·보관에 해당</b>하며, 서비스
            이용 시점에 정보통신망을 통해 이루어집니다.
          </p>
          <ul className="mt-2 list-disc space-y-1 pl-5">
            {PROCESSORS.map((p) => (
              <li key={p.name}>
