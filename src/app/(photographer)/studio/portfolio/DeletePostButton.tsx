...
export function DeletePostButton({ albumId, count }: { albumId: string; count: number }) {
  return (
    ...
    >
      ...
      <button className="inline-flex h-7 items-center whitespace-nowrap rounded-full px-3 text-xs font-medium text-brand-ink transition-colors hover:bg-brand/[0.06]">
        게시물 삭제
      </button>
    </form>
  );
