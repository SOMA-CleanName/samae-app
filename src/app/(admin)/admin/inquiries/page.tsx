...
export default async function AdminInquiriesPage({
  ...
  const [{ data }, account] = await Promise.all([
    admin
      .from("inquiries")
      .select(
        "id, status, created_at, photographer_id, purpose, preferred_date, region, name, gender, party_size, note, deposit_amount_krw, deposit_confirmed_at, phone, kakao_id, contact_email, ref_image_paths, fbp, fbc, source_photo_id, utm_source, utm_medium, utm_campaign, landing_path, hidden_from_photographer, is_test, photographer:photographers(display_name), profile:profiles!inquiries_profile_id_fkey(display_name)"
      )
