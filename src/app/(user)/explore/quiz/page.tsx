...
export default async function TasteQuizPage() {
  ...
  const photos = await listDiverseQuizPhotos(15);

  return (
    <section className="px-2.5 pb-20 pt-3 font-kr sm:px-4 sm:pt-4">
      <MpTrackOnce event="View Taste Quiz" />
      <Link
        href="/explore"
        aria-label="탐색으로 돌아가기"
        className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-line bg-surface text-fg shadow-card transition-colors hover:bg-surface-2"
      >
