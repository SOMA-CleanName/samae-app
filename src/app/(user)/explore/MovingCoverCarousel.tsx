...
export function MovingCoverCarousel({ cats }: { cats: CoverCat[] }) {
  ...
  // step 이 바뀔 때마다 리셋되는 타이머 — 항상 STEP_MS(2초)로 자동넘김.
  // 게이지도 STEP_MS 로 채워져(아래) 넘김과 항상 동기 — 스와이프/탭 후에도 동일하게 2초.
  useEffect(() => {
  ...
  const goToCat = (dir: number) => {
    ...
    setStep(catStart[target] ?? 0); // 스와이프 후에도 자동넘김은 새 step 기준 2초로 리셋
  };
