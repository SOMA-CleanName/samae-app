...
/*
  ...
  🔗 **환불·연락처 규정은 네 곳이 같은 말을 한다. 한 커밋에서 함께 고칠 것.**
       · 이 파일 (약관 조문)
       · `src/lib/refund.ts`              (환불 판정 로직)
       · `src/lib/platform-policy.ts`     (봇이 고객에게 하는 설명 — dev)
       · `docs/32-refund-policy.md` §1·§3-3 (정책 원본)
     하나만 고치면 나머지 셋이 **회사의 다른 약속**으로 남는다. 실제로 그런 상태가 있었다 —
     platform-policy 는 "연락처는 작가에게 공개되지 않는다" 고 하는데 처리방침 §3 은
     "작가에게 연락처를 제공합니다" 라고 말하고 있었다.
  ...
  🔴 **PG 를 붙이면 제4조 2항이 거짓이 된다** — 돈이 회사를 거치게 되기 때문이다.
     ...
     **개정안은 이미 써 뒀다 — `docs/37-terms-pg-amendment.md`.** 조문 전문·처리방침
     ...
     ⚠️ **대기 기간이 일정을 막지 않는다.** 한때 "불리한 변경 30일" 로 잡았다가 고쳤다 —
        회원을 세어 보면 순수 고객 회원이 2명이고 문의의 92%가 비회원이다. 비회원은 약관
        당사자가 아니고, 신규 가입자는 가입 시 동의로 즉시 적용된다.
        **실제로 필요한 것은 작가 17명에 대한 사전 고지**이고 그건 별도 문서로 진행한다.
*/
