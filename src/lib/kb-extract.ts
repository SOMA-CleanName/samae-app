...
[불일치가 **아닌** 것 — 여기서 대부분이 걸러집니다]
작가가 보낸 자료는 대개 **요약**이고, 사매 패키지는 그 **상세**입니다. 둘이 다르게 보이는
건 대부분 어긋난 게 아니라 같은 말을 다른 크기로 쓴 것입니다. 아래는 묻지 마세요.
- **요약 ↔ 상세.** 자료에 "세부보정 할인" 한 줄이고 패키지에는 "리퀴파이 할인",
  "보정본 2장 추가" 처럼 나뉘어 적혀 있으면, 자료가 그걸 묶어 쉽게 쓴 것입니다.
  상세가 맞습니다.
- **기본값 ↔ 상품별 값.** 자료의 "기본옵션 보정본 6장" 은 대표 구성이고, 상품마다
  6·8·10장으로 다른 건 정상입니다. 다만 **어느 상품이 기본인지** 자료로 알 수 없으면
  그건 물어도 됩니다.
- **최저가 ↔ 더 싼 다른 상품.** 자료가 "개인스냅 50,000원부터" 라는데 40,000원짜리
  상품이 따로 있으면, 그건 용도가 다른 상품입니다(체험용·프로필 특화 등).
  **묻기 전에 그 상품의 설명을 읽으세요 — 대개 거기에 답이 적혀 있습니다.**
- **상한 ↔ 작가가 정한 기준.** 사매가 정한 기한·한도 안에서 작가가 더 짧게 잡는 것은
  어긋난 게 아닙니다(납품 21일 한도에 작가가 10일 기준).
- **명백한 오타.** "4~60분" 은 40~60분입니다. 숫자가 상식적으로 불가능하면 바로잡아
  카드에 쓰고, 묻지 마세요.
읽는 사람은 작가입니다. **안 물어도 되는 걸 물으면 성의 없어 보입니다.**
자료에 이미 답이 있는데 되묻는 것은 자료를 안 읽었다는 뜻입니다. 묻기 전에 패키지 설명을
끝까지 읽으세요 — 금액 차이의 이유는 대개 그 상품 설명 안에 적혀 있습니다.

[자료와 등록값이 겹칠 때 — 캐묻지 말고 **제안**하세요]
같은 항목을 양쪽이 조금씩 다르게 적고 있으면, 항목마다 "이게 맞나요" 를 늘어놓지 마세요.
작가는 같은 확인을 여러 번 받는 셈이고, 안내물 하나 만드는 데 취조받는 느낌이 됩니다.
**"등록해두신 패키지 기준으로 정리해드려도 될까요"** 한 문장으로 묶어 확인만 받으세요.
그 한 문장을 questions 의 **맨 앞**에 두고, 뒤에는 **자료에도 패키지에도 없어서 정말 모르는
것**만 남깁니다.

[질문을 만드는 기준]
물어볼 것은 두 가지뿐입니다. 그 밖의 질문은 만들지 마세요.
1. 위에서 찾은 불일치 — 단, 바로 위 "불일치가 아닌 것" 을 먼저 걸러낸 뒤
2. 아래 핵심 주제 중 자료에 답이 없는 것: ${COVERAGE_TOPICS.join(", ")}
질문은 작가에게 그대로 보낼 수 있게, 정중한 존댓말 한두 문장으로 씁니다.
무엇이 어떻게 다른지 근거를 함께 적어주세요.

**작가의 선택을 캐묻지 마세요.** 동의하지 않은 항목, 켜지 않은 설정, 올리지 않은 정보는
작가가 그렇게 정한 것입니다. 이유를 묻거나 바꾸도록 권하는 질문은 만들지 않습니다.
**해줄 수 없는 것을 약속하지 마세요.** "알아보겠다", "만들어 보겠다" 같은 말을 붙이지 않습니다.
