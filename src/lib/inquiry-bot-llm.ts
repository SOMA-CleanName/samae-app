...
export const llmSlotsSchema = z.object({
  purpose: z.string().optional().describe("촬영 목적 (예: 커플·우정 스냅, 웨딩)"),
  preferredDate: z
    .string()
    .optional()
    .describe("촬영 희망일 — 구체적 날짜면 yyyy-mm-dd, 아니면 사용자가 말한 표현 그대로"),
  region: z.string().optional().describe("촬영 지역 (예: 서울, 경기·인천)"),
  partySize: z.string().optional().describe("촬영 인원 (예: 2명, 3~6명)"),
  custom: z
    .record(z.string())
    .optional()
...
export const botTurnSchema = z.object({
  ...
  asking: askingSchema.describe("이번 답변에서 묻고 있는 주제 (질문이 아니면 none)"),
  done: z.boolean().describe("필수 슬롯 수집과 커스텀 질문이 끝나 요약 단계로 넘어가도 되는지"),
});
export type BotTurn = z.infer<typeof botTurnSchema>;

/** API 응답 — 핸드오프면 봇 정지 신호만 보낸다 */
export type BotApiResponse = (BotTurn & { handedOff?: false }) | { handedOff: true };
...
// 이전 슬롯 값을 병합해 수집 상태가 후퇴하지 않게 한다.
export function sanitizeBotTurn(turn: BotTurn, prevSlots: LlmSlots): BotTurn {
  const mergedCustom = { ...prevSlots.custom, ...turn.slots.custom };
  const slots: LlmSlots = {
    ...prevSlots,
    ...Object.fromEntries(
  ...
  return {
    ...
    done: turn.done && coreSlotsFilled(slots),
  };
}
...
- 촬영 문의와 무관한 질문(작가 사생활, 다른 주제)은 정중히 문의 흐름으로 돌린다. 가격·환불 등 정책성 답변은 단정하지 말고 "작가님이 직접 안내드릴 거예요"로 넘긴다.`;
}
