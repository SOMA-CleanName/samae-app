 ...
 *
 * 1) **법정 의무** — 전자상거래법 제10조. 사이버몰 운영자는 아래를 **초기화면에** 표시해야 한다.
 *      상호 · 대표자 성명 · 영업소 소재지 주소 · 전화번호 · 모사전송번호 · 전자우편주소 ·
 *      사업자등록번호 · **사이버몰 이용약관** · 그 밖에 소비자보호에 필요한 사항
 *    ⚠️ **이용약관 링크도 법정 표시 항목이다.** 이 블록이 아니라 SiteFooter·SiteLinksRow 의
 *       링크 목록이 담당한다 — 거기서 /terms 를 빼면 의무 미이행이 된다.
 *    ⚠️ "초기화면" 이라 홈에도 있어야 한다. 홈은 무한 스크롤이라 푸터에 안 닿으므로
 *       SiteLinksRow 가 그 자리를 맡는다.
